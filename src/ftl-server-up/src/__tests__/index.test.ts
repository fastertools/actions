...
>
const mockedKillProcessGracefully =
  killProcessGracefully as jest.MockedFunction<typeof killProcessGracefully>
...
describe('FTL Server Up Action', () => {
  ...
  describe('Process Management', () => {
    ...
    test('captures and logs upr output', async () => {
      const outputProcess = {
        ...
        stdout: {
          on: jest.fn().mockImplementation((event, callback) => {
            if (event === 'data') {
              setTimeout(
                () => callback(Buffer.from('Server started successfully\n')),
                50
              )
            }
          })
      ...
      expect(mockedCore.info).toHaveBeenCalledWith(
        'Server stdout: Server started successfully'
      )
      expect(mockedCore.info).toHaveBeenCalledWith(
        'Server stderr: Loading configuration...'
      )
    })
  })
  ...
  describe('Health Check Integration', () => {
    ...
    test('handles health check failure', async () => {
      ...
      expect(mockedKillProcessGracefully).toHaveBeenCalledWith(
        mockProcess,
        expect.any(Object)
      )
    })
    ...
    test('sets custom environment variables from input', async () => {
      mockedCore.getInput.mockImplementation((name: string) => {
        const inputs: Record<string, string> = {
          port: '8080',
          'env-vars':
            'DATABASE_URL=postgres://localhost:5432/ftl,LOG_LEVEL=info'
        }
        return inputs[name] || ''
      })
  ...
  describe('Output Generation', () => {
    ...
    test('includes process information in summary', async () => {
      ...
      expect(mockedCore.info).toHaveBeenCalledWith('Server process ID: 12345')
    })
  })
  ...
  describe('Cleanup and Error Handling', () => {
    ...
    test('handles cleanup function registration', async () => {
      ...
      const cleanupFn = mockProcessOn.mock.calls.find(
        (call) => call[0] === 'exit'
      )?.[1] as Function
