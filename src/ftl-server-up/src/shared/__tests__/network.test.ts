  ...
  describe('downloadWithRetry', () => {
    ...
    test('throws error after exhausting all retry attempts', async () => {
      ...
      await expect(promise).rejects.toThrow(
        'Download failed after 2 attempts: Persistent network error'
      )
    ...
    test('handles HTTP error responses properly', async () => {
      ...
      await expect(promise).rejects.toThrow(
        'Download failed after 1 attempts: HTTP 404: Not Found'
      )
      expect(mockFetch).toHaveBeenCalledTimes(1)
    })
    ...
    test('enforces timeout on slow requests', async () => {
      ...
      const promise = downloadWithRetry(
        'https://slow.example.com/file.tar.gz',
        '/tmp/file.tar.gz',
        options
      )
    ...
    test('creates directory if it does not exist', async () => {
      ...
      await downloadWithRetry(
        'https://example.com/file.tar.gz',
        '/deep/nested/path/file.tar.gz'
      )

      expect(mockedFs.mkdir).toHaveBeenCalledWith('/deep/nested/path', {
        recursive: true
      })
    })
  ...
  describe('checkUrlExists', () => {
    test('returns true for existing URL (HEAD request succeeds)', async () => {
      ...
      const exists = await checkUrlExists(
        'https://example.com/existing-file.tar.gz'
      )
    ...
    test('returns false for non-existing URL (HEAD request fails)', async () => {
      ...
      const exists = await checkUrlExists(
        'https://example.com/missing-file.tar.gz'
      )
    ...
    test('returns false when network error occurs', async () => {
      ...
      const exists = await checkUrlExists(
        'https://unreachable.example.com/file.tar.gz'
      )
